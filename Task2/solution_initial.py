...
class BayesianLayer(nn.Module):
    ...
    def __init__(self, in_features: int, out_features: int, bias: bool = True):
        ...
        self.prior = None   ##here add either the univariate or the multivariate with diagonal gaussian 
        #self.prior = UnivariateGaussian(torch.ones(output_dim, input_dim) * torch.tensor(self.prior_mu), torch.ones(output_dim, input_dim) * torch.tensor(self.prior_sigma))
        ...
        #  )
        #self.weights_var_posterior = MyPosterior(
        #      nn.Parameter(mu_init_distribution.sample()*torch.ones(output_dim, input_dim, requires_grad=True)),
        #      nn.Parameter(torch.log(.075 * torch.ones(output_dim, input_dim, requires_grad=True)))
        ...
        if self.use_bias:
            ...
            #self.bias_var_posterior = MyPosterior(nn.Parameter(torch.zeros(output_dim, requires_grad=True)),nn.Parameter(torch.zeros(output_dim, requires_grad=True)))
