        ...
        for _ in progress_bar:
            ...
            for batch_idx, (batch_x, batch_y) in enumerate(train_loader):
                ...
                if isinstance(self.network, DenseNet):
                    ...
                    current_logits, full_log_prior, full_log_var_posterior = self.network(batch_x)
                    #log_probs_per_class = torch.log1p(F.softmax(raw_logits, dim=1))

                    sum_log_prob_correct_class = F.nll_loss(F.log_softmax(current_logits, dim=1), batch_y, reduction='sum')
                    # for row in range(batch_y.size()[0]):
                    #     sum_log_prob_correct_class += log_probs_per_class[row,batch_y[row]]
                    ...
                    #     print(name, torch.isfinite(param.data) )
...
class BayesianLayer(nn.Module):
    ...
    def __init__(self, in_features: int, out_features: int, bias: bool = True):
        ...
        # NOTE: CHOICE OF CONSTANTS!
        self.weight_prior_mu  = torch.Tensor(out_features, in_features).uniform_(-0.2, 0.2)
        self.weight_prior_rho = torch.Tensor(out_features, in_features).uniform_(-3, -2)
        # self.weight_prior_mu  = torch.ones(out_features, in_features) * torch.tensor(0)
        # self.weight_prior_rho = torch.ones(out_features, in_features) * torch.tensor(.1)
        self.weight_prior = MultivariateDiagonalGaussian(self.weight_prior_mu, self.weight_prior_rho)

        if self.use_bias:
            self.bias_prior_mu  = torch.Tensor(out_features).uniform_(-0.2,0.2)
            self.bias_prior_rho = torch.Tensor(out_features).uniform_(-3, -2)
            # self.bias_prior_mu  = torch.ones(out_features) * torch.tensor(0)
            # self.bias_prior_rho = torch.ones(out_features) * torch.tensor(.1)
            self.bias_prior = MultivariateDiagonalGaussian(self.bias_prior_mu, self.bias_prior_rho)
        else:
        ...
        self.weights_var_posterior_mu = nn.Parameter(torch.Tensor(out_features, in_features).uniform_(-0.2, 0.2), requires_grad=True)
        self.weights_var_posterior_rho = nn.Parameter(torch.Tensor(out_features, in_features).uniform_(-5,-4), requires_grad=True)
        self.weights_var_posterior = MultivariateDiagonalGaussian(self.weights_var_posterior_mu, self.weights_var_posterior_rho)
        ...
        if self.use_bias:
            ...
            self.bias_var_posterior_mu = nn.Parameter(torch.Tensor(out_features).uniform_(-0.2, 0.2), requires_grad=True)
            self.bias_var_posterior_rho = nn.Parameter(torch.Tensor(out_features).uniform_(-5,-4), requires_grad=True)
            self.bias_var_posterior = MultivariateDiagonalGaussian(self.bias_var_posterior_mu, self.bias_var_posterior_rho)
    ...
    def forward(self, inputs: torch.Tensor):
        ...
        if self.use_bias :
            ...
            self.log_prior = self.weight_prior.log_likelihood(weights) + self.bias_prior.log_likelihood(bias)
            self.log_variational_posterior = self.weights_var_posterior.log_likelihood(weights) + self.bias_var_posterior.log_likelihood(bias)
        else:
            bias = None

            # NOTE: log prior / psoterior maybe *128 ???
            self.log_prior = self.weight_prior.log_likelihood(weights)
            self.log_variational_posterior = self.weights_var_posterior.log_likelihood(weights)

        return F.linear(inputs, weights, bias), self.log_prior, self.log_variational_posterior
...
class BayesNet(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> typing.Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        current_features       = x
        full_log_prior         = 0
        full_log_var_posterior = 0

        for idx, current_layer in enumerate(self.layers):
            new_features, log_prior, log_var_posterior = current_layer(current_features)
            full_log_prior         += log_prior
            full_log_var_posterior += log_var_posterior

            if idx < len(self.layers) - 1:
                new_features = self.activation(new_features)
            current_features = new_features

        return current_features, full_log_prior, full_log_var_posterior
...
class UnivariateGaussian(ParameterDistribution):
    ...
    def sample(self) -> torch.Tensor:
        # TODO: Implement this
        eps = torch.distributions.Normal(0, 1).sample(self.mu.size())
...
class MultivariateDiagonalGaussian(ParameterDistribution):
    ...
    def sample(self) -> torch.Tensor:
        eps = torch.distributions.Normal(0, 1).sample(self.mu.size())
        return self.mu + torch.log1p(torch.exp(self.rho)) * eps
