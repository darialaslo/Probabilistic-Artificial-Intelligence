...
##############################################################################
### justguessing: Daria L Auguste R Eljas R
...
class Model(object):
    ...
    def __init__(self):
        ...
        self.num_epochs = 95  # number of training epochs
        self.batch_size = 128  # training batch size
        learning_rate = 1e-3  # training learning rates
        hidden_layers = (120, 120)  # for each entry, creates a hidden layer with the corresponding number of units
        ...
        for _ in progress_bar:
            ...
            for batch_idx, (batch_x, batch_y) in enumerate(train_loader):
                ...
                if isinstance(self.network, DenseNet):
                    ...
                    ### Perform forward pass through our BNN
                    raw_logits, full_log_prior, full_log_var_posterior = self.network(batch_x)

                    ### Calculate negative log likelihood loss using our predictions.
                    sum_log_prob_correct_class = F.nll_loss(F.log_softmax(raw_logits, dim=1), batch_y, reduction='sum')
                    
                    ### Entire loss including log variational posterior prob. and log prior prob
                    loss = (full_log_var_posterior-full_log_prior)/num_batches+sum_log_prob_correct_class
...
class BayesianLayer(nn.Module):
    ...
    """
    ### Turns out this choice of priors is sufficient
    PRIOR_MU = torch.tensor(0)
    PRIOR_SIGMA1 = torch.tensor(1)
    # PRIOR_SIGMA2 = torch.tensor(0.0001)
    ...
    def __init__(self, in_features: int, out_features: int, bias: bool = True):
        ...
        # self.prior_generator = torch.Generator().manual_seed(42)
        self.weights_generator = torch.Generator().manual_seed(42)
        self.bias_generator = torch.Generator().manual_seed(43)
        ...
        ## Tried different values for the priors
        # self.prior = ScaleMixture(self.PRIOR_MU, sigma1 = self.PRIOR_SIGMA1, sigma2 = self.PRIOR_SIGMA2)
        self.prior = UnivariateGaussian(self.PRIOR_MU,self.PRIOR_SIGMA1)
        # self.prior = MultivariateDiagonalGaussian(
        #     mu=torch.zeros(out_features,in_features),
        #     rho=torch.ones(out_features,in_features)#.uniform_(-2, -1, generator = self.prior_generator)
        #     )
        ...
        ### Tried different parameters of initializing the variational posterior.
        ### For reproducibility use a generator with fixed manual seed
        self.weights_var_post_mu = nn.Parameter(torch.zeros(out_features, in_features).normal_(mean=0,std=1/20, generator = self.weights_generator))
        self.weights_var_post_rho = nn.Parameter(torch.Tensor(out_features, in_features).uniform_(-5, -3, generator = self.weights_generator))
        self.weights_var_posterior = MultivariateDiagonalGaussian(self.weights_var_post_mu, self.weights_var_post_rho)
        ...
        if self.use_bias:
            ...
            ### Use exactly identical parameters as for weight variational posterior but different generator
            self.bias_var_post_mu = nn.Parameter(torch.zeros(out_features).normal_(mean=0,std=1/20, generator = self.bias_generator))
            self.bias_var_post_rho = nn.Parameter(torch.Tensor(out_features).uniform_(-5, -3, generator = self.bias_generator))
            self.bias_var_posterior = MultivariateDiagonalGaussian(self.bias_var_post_mu, self.bias_var_post_rho)
    ...
    def forward(self, inputs: torch.Tensor):
        ...
        ### Sample weights from current weight variational posterior
        weights = self.weights_var_posterior.sample()
         
        ### Check if using bias or not
        ### This also affects the log variational posterior prob.
...
class BayesNet(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> typing.Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        ### Use the DenseNet forward pass as template. Add the passing of the log probabilites
        current_features = x
        for idx, current_layer in enumerate(self.layers):
            ### Add up the log probabilities of each layer
            current_features, layer_log_prior, layer_log_var_posterior = current_layer(current_features)
...
class UnivariateGaussian(ParameterDistribution):
    ...
    def __init__(self, mu: torch.Tensor, sigma: torch.Tensor):
        ...
        self.generator = torch.Generator().manual_seed(44)

    def log_likelihood(self, values: torch.Tensor) -> torch.Tensor:
        # TODO: Implement this
        ### Vanilla log-likelihood of pytorch
        return torch.distributions.Normal(self.mu, self.sigma).log_prob(values).sum()

    def sample(self) -> torch.Tensor:
        # TODO: Implement this
        ### Diagonal -> sample = mean + sigma * N(0,1)
        return self.mu + torch.randn(size=self.mu.shape) * self.sigma  #, generator = self.generator)
...
class MultivariateDiagonalGaussian(ParameterDistribution):
    ...
    def __init__(self, mu: torch.Tensor, rho: torch.Tensor):
        ...
        self.rho = rho
        ### Generator for reproducibility
        self.generator = torch.Generator().manual_seed(45)


    def log_likelihood(self, values: torch.Tensor) -> torch.Tensor:
        # TODO: Implement this
        
        ### Recover sigma from rho
    ...
    def sample(self) -> torch.Tensor:
        ...
        ### Recover sigma from rho
        sigma = torch.log(1 + torch.exp(self.rho))
        ### Diagonal Multivariate Gaussian Distribution -> sample = mean + sigma * N(0,1)
        return self.mu + sigma * torch.randn(size = self.mu.shape)#, generator = self.generator)


#############################################################################
### Scale Mixture 
class ScaleMixture(ParameterDistribution):
    PI = 0.5 # To be optimzized
    def __init__(self, mu: torch.Tensor, sigma1: torch.Tensor, sigma2: torch.Tensor):
        super(ScaleMixture, self).__init__()  # always make sure to include the super-class init call!
        assert mu.size() == sigma1.size()
        self.mu = mu
        self.sigma1 = sigma1
        self.sigma2 = sigma2
        
        self.Prior1 = UnivariateGaussian(mu, sigma1)
        self.Prior2 = UnivariateGaussian(mu, sigma2)
        
    def log_likelihood(self, values: torch.Tensor) -> torch.Tensor:
        return self.PI * self.Prior1.log_likelihood(values) + (1-self.PI) * self.Prior2.log_likelihood(values)
    
    def sample(self) -> torch.Tensor:
        return self.PI * self.Prior1.sample() + (1-self.PI) * self.Prior2.sample()
#############################################################################
