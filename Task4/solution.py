...
class MLPActorCritic(nn.Module):
    ...
    def step(self, state):
        ...
        with torch.no_grad():
            ...
            log_prob = self.pi._log_prob_from_distribution(pi, action)
...
class VPGBuffer:
    ...
    def end_traj(self, last_val=0):
        ...
        self.ret_buf[path_slice] = discount_cumsum(rews[:-1], self.gamma)[0] * np.ones(self.ptr-self.path_start_idx)
    ...
    def get(self):
        ...
        #standard normalisation, subtract the mean and divide by standard deviation
...
class Agent:
    ...
    def pi_update(self, data):
        ...
        #for the update I just changed ret to phi in the loss function 
        ...
        _, log_prob = self.ac.pi.forward(obs, act)
        loss = - (phi * log_prob).mean()
    ...
    def get_action(self, obs):
        ...
        #extract policy distribution from your trained and updated policy, given the observations
        policy = self.ac.pi._distribution(torch.from_numpy(obs).float())

        #extract logits and identify which action gives you the highest log probability
        action = policy.logits.argmax().item()

        #output the best action given the probabilities
        return action
