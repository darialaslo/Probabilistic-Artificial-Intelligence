...
class BO_algo(object):
    def __init__(self):
        ...
        ######
        #defining kernels
        self.constraint_kernel = ConstantKernel(3.5) * RBF(2.0)
        self.objective_kernel = ConstantKernel(1.5) * RBF(1.5)
        #defining the noise params
        self.constraint_noise = 0.005
        self.objective_noise = 0.01
        #define oarameter for upper confidence bound
        self.beta = 5.0
        self.gamma = 10.0 #how much the constraint affects our choice
        ######
        ...
        self.constraint_model = GaussianProcessRegressor(kernel = self.constraint_kernel, alpha = self.constraint_noise**2, random_state=0)  # TODO : GP model for the constraint function
        self.objective_model = GaussianProcessRegressor(kernel = self.objective_kernel, alpha = self.objective_noise**2, random_state=0) # TODO : GP model for your acquisition function
    ...
    def acquisition_function(self, x: np.ndarray) -> np.ndarray:
        ...
        # TODO: enter your code here
        #I suggest using upper confidence bound at the beginning, somehow including the constraint function as well
        #print(x.shape)
        #assert x.shape == (1,2)

        # if len(self.previous_points) == 0:
        if not hasattr(self, 'data'):
            mu_constraint , sigma_constraint = self.constraint_model.predict(x.reshape(1,-1), return_std=True)
            mu_objective, sigma_objective = self.objective_model.predict(x.reshape(1,-1), return_std=True)
        else:
            mu_constraint , sigma_constraint = self.constraint_model.predict(x.reshape(1,-1),return_std=True)
            mu_objective, sigma_objective = self.objective_model.predict(x.reshape(1,-1),return_std=True)

        return  (mu_objective - self.gamma * mu_constraint) + self.beta * (sigma_objective + sigma_constraint)
    ...
    def add_data_point(self, x: np.ndarray, z: float, c: float):
        ...
        if len(self.previous_points) == 1:
            ...
            #retrain and update
            # print('fitting')
            current_c = np.array(self.previous_points)[:,3]
            current_z = np.array(self.previous_points)[:,2]
            self.constraint_model.fit(X = np.array(self.previous_points)[:,:2], y = current_c)
            self.objective_model.fit(X = np.array(self.previous_points)[:,:2], y = current_z)
            # print('done fitting')
    ...
    def get_solution(self) -> np.ndarray:
        ...
        samples =np.array(self.previous_points)[:,2]
        #samples[np.array(self.previous_points)[:,3] < 1.2] = -1e6
